import styled from "styled-components";
// import GlobalStyle from '../../components/GlobalStyle';
import {Helmet} from 'react-helmet';
import MainContainer from '../../components/MainContainer';
import Topbar from '../../components/Topbar';
import PageContents from '../../components/PageContents';

const AboutMeDiv = styled.div`
    font-family: "default_sans";
    color: white;
    text-align: left;
`;

const ProfilePic = styled.img`
    position: relative;
    float: right;
    top: 5px;    
    padding: 7px 10px 10px 7px;
`;

const TopbarEntriesAboutMe = {
    "About": {
        "About Me": "/about/about_me",
        "Links": "/about/links"
    },
    "Cool Places": {
        "Favorite Websites": "/places/favorite_sites"
    },
    "Projects": {
        "Current Projects": "/projects/current_projects",
        "Past Projects": "/projects/past_projects",
        "Projects I wish I had time to work on": "/projects/piwihttwo"
    }
};

// <GlobalStyle />

export default function about_me () {
    return (
        <>
            <Helmet>
              <title>About Me</title>
            </Helmet>
            <MainContainer>
                <Topbar entries={TopbarEntriesAboutMe}/>
                <PageContents>
                    <AboutMeDiv>
                        <h2>About Me</h2>
                        <ProfilePic src="data:image/jpeg;base64,"></ProfilePic>
                        <p>
                            Hey it's Vsauce here. Actually my name is Jacob. I am an alumnus of the University of Tennessee at Chattanooga with a Bachelor's degree in Computer Science: Software Systems 
                            and a minor in Economics. In my free time I enjoy mountain biking, gaming, programming, and 
                            tinkering/repairing electronics and cars. I'm not a huge fan of social media so I don't really post much online. 
                        </p>
                        <p>
                            Regarding programming, I have experience in several languages such as Python, Java, JavaScript, C#, C, PHP, SQL, Visual Basic and Lua. Recently I have been 
                            dabbling in some data engineering technologies such as Spark, Kafka, and dbt. In my current role I've found a passion for creating and architecting parts of 
                            data pipelines. I've also been experimenting with Agentic AI using OLLAMA as the AI interface of choice for some AI workflows. I'm particularly excited to take on some
                            professional and personal projects using OLLAMA. So far, OLLAMA has proven to be very powerful and cost effective. I'm stoked to hit the ground running on some of these projects. 
                        </p>
                        <p>
                            Anyways, I'm excited for what the future has to offer! I love learning new things, seeing new places, and meeting 
                            new people. I'm not entirely sure what the main purpose of this website will be other 
                            than as my sort of personal website blog thing. I'll probably experiment around with it when I'm bored or not working on one of many of my stupid projects. 
                            As George Carlin once said, "I have lots of ideas. Trouble is, most of them suck."
                        </p>
                        <br></br>
                        <p>
                            56 65 6E 69 2C 20 76 69 64 69 2C 20 76 69 63 69 2C 20 76 69 78 69
                        </p>
                    </AboutMeDiv>
                </PageContents>
            </MainContainer>
        </>
    )
}
